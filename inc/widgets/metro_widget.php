 ...
 */
// Register the widget
add_action( 'widgets_init', create_function( '', 'return register_widget("Techfind_Metro_Widget_Block");'));
// The widget class
class Techfind_Metro_Widget_Block extends WP_Widget {
	public function __construct() {
		$widget_ops = array('classname' => 'metro_widget', 'description' => esc_html__( "Display posts use on homepage.", 'techfind') );
		parent::__construct('cs_metro', esc_html__('Metro Widget', 'techfind'), $widget_ops);
		$this->alt_option_name = 'widget_metro';
	...
	public function widget( $args, $instance ) {
		...
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_metro', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
		...
		<div class="metro_widget_content">
			<?php while ( $custom_query->have_posts() ) : $custom_query->the_post(); $count++; ?>
        		...
        		?>
                    ...
                    <!-- begin .hentry -->
                    <article id="post-<?php the_ID(); ?>" <?php post_class( 'metro-post '.$class ); ?> >

                        <!-- begin .featured-image -->
                        <?php if ( has_post_thumbnail() ) { ?>
                        <div class="featured-image">
                            <?php if ( has_post_thumbnail() ) : ?><?php the_post_thumbnail( $thumb ); ?><?php endif; ?>
                            <div class="metro_overlay"></div>
                            ...
                            <div class="entry-content">
                                ...
                                <div class="entry-meta">
                                    <?php
									if ( $count == 1 || $count == 6 ) :
										techfind_posted_on_2();
									endif;
									?>
                                </div>
                            </div>

                            <a class="metro_link" href="<?php the_permalink() ?>" title="<?php the_title() ?>"></a>
                        </div>
                        <?php } ?>
                        <!-- end .featured-image -->
                    </article>
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_metro', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
	...
	public function remove_cache() {
		wp_cache_delete('widget_metro', 'widget');
	}
	/**
	 * @param array $instance
	 */
